    ...
    <div class="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
      <div class="bg-gray-800 rounded-lg p-6 h-[500px] w-full max-w-md border border-gray-600 flex flex-col">
        ...
        <div class="flex flex-col md:flex-row gap-3 relative">
          ...
          <div
            *ngIf="showSuggestions && results.length"
          ...
          >
            <div
              *ngFor="let item of results"
              ...
              class="px-4 py-3 cursor-pointer hover:bg-gray-600 border-b border-gray-600"
            >
        ...
        <p *ngIf="error" class="text-red-400 mt-3 text-sm">{{ error }}</p>

        <div class="mt-auto pt-4">
          <button
            (click)="close.emit()"
            class="w-full px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-500"
          >Cancel</button>
        </div>
      </div>
    </div>
